...
meancv <- function(x){
  ...
  # first one is name, later one is the value from the above lines
  df <- data.frame(mean = mean,
                   cv = cv
                   )
  # Return the data frame
  #or you can just type df
  return(df)
}
...
nematode_mods <- nematode_w %>%
  ...
  pivot_longer(cols = mel_s:eggs_r,
               names_to = "resp_var", #response variables names
               values_to = "resp_val"
               ) %>%
  ...
  nest() %>% # tibble here, holds all the values, but just put them into one tibble
  ...
  mutate(r2 = map_dbl(lm,
                  ~summary(.x)$r.square
                  )) #map_dbl can pull out the r2 dbl values out of the nest dataframe
  
nematode_mods
nematode_mods$data[3] # to see the exact tibble, it's actually an another data.frame
